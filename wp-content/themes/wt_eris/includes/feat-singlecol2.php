 ...
 *
 * @package  WellThemes
 * @file     feat-singlecol2.php
 * @author   Well Themes Team
 * @link 	 http://wellthemes.com
 */
?>
<?php	
	$sf2_cat_id = wt_get_option('wt_sf_cat2');														//get category id		
	
	if ( $sf2_cat_id == 0 ) {																		//no category is selected, get latest posts
		...
		$post_query1 = 'cat='.$sf2_cat_id.'&posts_per_page=1&ignore_sticky_posts=1';				//get  main post
		$post_query2 = 'cat='.$sf2_cat_id.'&posts_per_page=3&offset=1&ignore_sticky_posts=1';		//get 3 more posts
		$sf2_cat_name = get_cat_name($sf2_cat_id);													//get category name
		$sf2_cat_url = get_category_link($sf2_cat_id );												//get category url.
	}	
?>
<div class="category one-half last-col">

	<h3>
		<?php
				if ($sf2_cat_id == 0 ) {									//no category is selected, display text latest.
					 _e('Latest Posts', 'wellthemes');						
				} else {													//display category name and url
					?>
					<a href="<?php echo esc_url( $sf2_cat_url ); ?>" ><?php echo $sf2_cat_name; ?></a>
					<?php
				}						
			?>
	</h3>
